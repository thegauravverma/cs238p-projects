...
static void
generate_sigmoid(const struct parser_dag *dag, FILE *file)
{
	fprintf(file, "double evaluate(void) {\n");
	fprintf(file, "typedef double (*sigmoid_t)(double);\n");
	fprintf(file, "sigmoid_t sigmoid_f;\n");
	fprintf(file, "long func_ptr = %ld;\n",(long)sigmoid);
	reflect(dag, file);
	fprintf(file, "sigmoid_f = (sigmoid_t)func_ptr;\n");
	fprintf(file, "return sigmoid_f(t%d);\n}\n", dag->id);
}
...
main(int argc, char *argv[])
{
	...
	if(argc == 3 && strcmp("sigmoid", argv[2]) == 0) {
		generate_sigmoid(parser_dag(parser), file);
	} else {
